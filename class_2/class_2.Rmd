...
.img-right[![](images/class_2/poppins-bag-kids.gif)]
...
background-image: url("images/class_2/typewriter.jpg")
...
- A diferencia de otros softwares estadísticos (e.g, `SPSS`, `SAS`,`Stata`), en `R` las bases de dato son un tipo de estructura de datos, entre muchos otros.
  ...
  - Un dataframe es una matriz donde cada file es una "observación y cada columna es un "variable". 
...
Utilizamos doble corchetepara acceder a cada elemento, o el operador `$`.
```{r}
mylist[[1]]
mylist$data[1:2,2:3]
```
...
.pull-right[
![NO](images/class_2/franja_NO.jpg)
]
...
Para inspeccionar rapidamente una base de datos podemos usar las funciones `head()` and `tail()`. Para saber las dimensiones de una matriz o data frame (*rows*, *cols*) usamos la función `dim()`.

```{r}
# Carganos los datos de Chile
library("carData")
...
Filas y columnas se seleccionan usando corchetes cuadrados para "indexar" la posición que se quiere seleccionar. Por ejemplo, `data_chile[3,8]` corresponde a observación 3, varible 8:
...
En un data frame los valores peridios aparecen como `NA`s. `NA`s son un tipo de dato.
.left-code[
...
Para todas las distribuciones estadísticas, `R` utiliza las siguiente nomenclatura:

- `d*` = función de densidad/masa
- `p*` = función de distribución acumulativa
- `q*` = función de cuantiles
- `r*` = generación de variaciones aleatorias

Hay muchas disponibles en los paquetes basales de R. Ejecutar `?distributions` para ver una lista completa.
...
.pull-left[
```{r}
my_normal <- rnorm(n=1000, mean=0, sd=1)
matrix(my_normal,10,3)
```
]
...
```{r, echo=FALSE}
plot(density(my_normal))
abline(v = qnorm(p=c(0.025,0.975), 
      mean=0,sd=1), col="red")
abline(v = quantile(my_normal, 
        p=c(0.025,0.975)), col="blue")
```
]
...
## Funciones básicas y variables aleatorias en prácticas

Calculemos la desviación estándar de la variable que creamos ( $N(0,1)$ ). Para ello usaremos la siguiente fórmula:

$$\sigma = \sqrt{ \frac{\sum_{i=1}^{n} (x_{i} - \bar{x})^{2}}{n} }$$
```{r}
n        <- length(my_normal)
mu_bar   <- mean(my_normal)
dev2     <- (my_normal - mu_bar)^2
sigma2   <- sum(dev2)/n
...
## Aplicaciones de loops, iteraciones y estructuras de control

***Infinitas***: cualquier repeticion manual puede ser automatizada
...
background-image: url("images/class_2/bicicleta.jpg")
...
![](images/class_2/stack-exchange-search.png)
